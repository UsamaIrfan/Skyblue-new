...
const searchProduct = (data) =>{
  ...
  const handleBarCodeScanned = async ({ type, data }) => {
    ...
    try {
      ...
      if(resData.Success === false) {
        Alert.alert('No Product Found', 'Sorry we are unable to find any product')
      }else{
        navigation.navigate('Shop',{screen: 'Detailed', params:{item: resData}, initial:false})
      }
  ...
  return (
    ...
    <View style={styles.container}>
        ...
        <Text style={{fontSize:19, fontFamily:'Regular', color:colors.Black}}>Scan Barcode</Text>
        <Text style={{fontSize:15, fontFamily:'Regular', color:colors.Grey, marginTop:5}}>Search Products with barcode..</Text>
        </View>
...
>
<Text style={{ fontSize: 16, color: "white" }}>Tap to Scan Again</Text>
</TouchableOpacity>}
      </View>
